...
pub(crate) fn expand(input: SqlFunctionDecl) -> Result<TokenStream, Diagnostic> {
    ...
    let mut tokens = quote! {
        ...
        #where_clause
            (#(#arg_name),*): Expression,
        {
            type SqlType = Typed<#return_type>;
        }
    ...
    if is_aggregate {
        ...
        if cfg!(feature = "sqlite") && type_args.is_empty() {
            tokens = quote! {
                ...
                use diesel::sql_types::{IntoNullable, Typed};
            };
                ...
                x if x > 1 => {
                    tokens = quote! {
                        ...
                        pub fn register_impl<A, #(#arg_name,)*>(
                            ...
                            (#(#arg_name,)*): FromSqlRow<Typed<(#(#arg_type,)*)>, Sqlite> +
                                StaticallySizedRow<Typed<(#(#arg_type,)*)>, Sqlite>,
                        {
