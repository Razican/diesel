...
use diesel::{backend::Backend, row::NamedRow};
...
impl<I> DynamicRow<I> {
    ...
    fn from_row<'a, DB>(row: &impl Row<'a, DB>) -> deserialize::Result<Self>
    where
        DB: Backend,
        I: FromSql<Any, DB>,
    {
...
impl<I> DynamicRow<NamedField<Option<I>>> {
    fn from_nullable_row<'a, DB>(row: &impl Row<'a, DB>) -> deserialize::Result<Self>
    where
        DB: Backend,
        I: FromSql<Any, DB>,
    {
