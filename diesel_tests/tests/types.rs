...
fn query_single_value<T, U: FromSqlRow<Typed<T>, TestBackend>>(sql_str: &str) -> U
where
...
where
    ...
    U::Expression: SelectableExpression<(), SqlType = Typed<T>>
        + ValidGrouping<(), IsAggregate = is_aggregate::Never>,
    U::Expression: QueryFragment<TestBackend> + QueryId,
    T: QueryId + SingleValue + SqlType,
    T::IsNull: MixedNullable<T::IsNull, Out = T::IsNull>,
    T::IsNull: MaybeNullableType<Bool>,
    <T::IsNull as MaybeNullableType<Bool>>::Out: SqlType,
    diesel::sql_types::NotNull: diesel::expression::operators::AllIsNullable<
        <<T::IsNull as MaybeNullableType<Bool>>::Out as SqlType>::IsNull,
        Out = NotNull,
    >,
    Eq<SqlLiteral<T>, U>: Expression<SqlType = Typed<<T::IsNull as MaybeNullableType<Bool>>::Out>>,
    And<IsNull<SqlLiteral<T>>, IsNull<AsExprOf<U, T>>>: Expression<SqlType = Typed<Bool>>,
{
    use diesel::dsl::sql;
    let connection = connection();
    let query = select(
